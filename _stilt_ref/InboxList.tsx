...
function SmartMailContent({ mail, showReasoning }: { mail: Mail; showReasoning?: boolean }) {
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {showReasoning && mail.aiReasoning && (
          <SmartReasoningChip text={mail.aiReasoning} />
        )}
      </div>
    </div>
  );
...
function SmartMailRow({
  ...
  return (
    ...
    >
      <SmartMailContent mail={mail} showReasoning={showReasoning} />
    </SwipeableRow>
  );
}
...
function SmartInboxView({ mails, setMailStatus, params }: { mails: Mail[]; setMailStatus: (id: string, s: any) => void; params?: { id?: string } | null }) {
  ...
  const pendingInvitesCount = mockEvents.filter((e) => {
    const eff = rsvpOverrides[e.id] ?? e.rsvpStatus;
    return eff === 'pending';
  }).length;
  const todayForYou = mails
    .filter((m) => m.priority === 'high' && m.status === 'open' && m.needsReply)
    .slice(0, 5);
  const quickToClear = mails.filter(
    (m) =>
      m.status !== 'done' &&
      (m.category === 'fyi' || (m.category === 'newsletter' && m.priority === 'low'))
  );
  const waiting = mails.filter((m) => m.status === 'waiting');
  const quieted = mails.filter(
    (m) => m.status !== 'done' && (m.category === 'promo' || (m.category === 'newsletter' && m.priority === 'low'))
  );

  const [quietedOpen, setQuietedOpen] = useState(false);
    ...
    <div className="flex flex-col gap-5 md:gap-6">
      ...
      >
        ...
        <p className="text-[15px] text-foreground/70 mt-2 leading-snug">
          <span className="font-semibold text-foreground">{todayForYou.length} mails</span> need
          your attention today.{' '}
          <span className="text-foreground/55">
            {quieted.length} were auto-quieted so you can focus.
          </span>
          {pendingInvitesCount > 0 && (
            <>
              {' '}
              <span className="font-semibold text-foreground">
                {pendingInvitesCount} calendar invite{pendingInvitesCount > 1 ? 's' : ''} need{pendingInvitesCount > 1 ? '' : 's'} your response.
              </span>
            </>
          )}
        </p>
      ...
      {todayForYou.length > 0 && (
        <section>
          <div className="flex items-center gap-2 px-2 mb-1.5">
            <span className="text-[12.5px] font-semibold tracking-[-0.005em]">Today for you</span>
            <span className="text-[12px] text-muted-foreground">{todayForYou.length}</span>
          </div>
          <div className="stilt-card rounded-3xl overflow-hidden">
            {todayForYou.map((m, i) => (
              <div key={m.id}>
                {i > 0 && <div className="ml-[64px] h-px bg-foreground/[0.06] dark:bg-white/[0.06]" />}
                <SmartMailRow mail={m} showReasoning setMailStatus={setMailStatus} active={activeId === m.id} />
              </div>
            ))}
          </div>
        </section>
      ...
      {quickToClear.length > 0 && (
        <section>
          <div className="flex items-center justify-between px-2 mb-1.5">
            <div className="flex items-center gap-2">
              <span className="text-[12.5px] font-semibold tracking-[-0.005em]">Quick to clear</span>
              <span className="text-[12px] text-muted-foreground">{quickToClear.length}</span>
            </div>
            <button
              data-testid="button-clear-all"
              onClick={() => quickToClear.forEach((m) => setMailStatus(m.id, 'done'))}
              className="text-[12px] font-semibold text-foreground/60 flex items-center gap-1 hover-elevate active-elevate-2 px-2 py-1 rounded-full"
            >
              <CheckCheck size={12} strokeWidth={2.2} /> Clear all
            </button>
          </div>
          <div className="stilt-card rounded-3xl overflow-hidden">
            {quickToClear.map((m, i) => (
              <div key={m.id}>
                {i > 0 && <div className="ml-[64px] h-px bg-foreground/[0.06] dark:bg-white/[0.06]" />}
      ...
      {waiting.length > 0 && (
        <section>
          <div className="flex items-center gap-2 px-2 mb-1.5">
            <span className="text-[12.5px] font-semibold tracking-[-0.005em]">Waiting on others</span>
      ...
      {quieted.length > 0 && (
        <section>
          <button
            data-testid="button-quieted-toggle"
            onClick={() => setQuietedOpen((o) => !o)}
            className="w-full glass rounded-3xl px-4 py-3.5 flex items-center justify-between hover-elevate active-elevate-2"
          >
            <div className="flex items-center gap-2">
              <VolumeX size={14} strokeWidth={2} className="text-icon-muted" />
              <span className="text-[13.5px] font-medium">Auto-quieted</span>
              <span className="text-[12.5px] text-muted-foreground">
                · {quieted.length} quieted today
              </span>
            </div>
            <motion.span animate={{ rotate: quietedOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
