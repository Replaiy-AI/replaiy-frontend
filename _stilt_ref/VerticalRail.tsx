...
const PRIMARY: PrimaryItem[] = [
  { key: 'inbox',    icon: Inbox,        href: '/',         label: 'Mail' },
  { key: 'calendar', icon: CalendarIcon, href: '/calendar', label: 'Calendar' },
  { key: 'docs',     icon: FileText,     href: '/docs',     label: 'Docs' },
];
...
export function VerticalRail() {
  ...
  const tab: 'inbox' | 'calendar' | 'docs' = loc.startsWith('/calendar')
    ? 'calendar'
    : loc.startsWith('/docs')
      ? 'docs'
      : 'inbox';

  // v31 — In de Mail-context gebruiken we SquarePen (universele compose-
  // affordance, zoals Apple Mail / Spark / Twitter). Calendar en Docs
  // blijven Plus omdat dat semantisch "add item" is.
  const newAction =
    tab === 'inbox'
      ? { href: '/compose',      label: 'New mail',  icon: SquarePen }
      : tab === 'calendar'
        ? { href: '/calendar/new', label: 'New event', icon: Plus }
        : { href: '/docs/new',     label: 'New doc',   icon: Plus };
