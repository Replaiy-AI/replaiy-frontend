...
function EmptyDetail() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
      ...
      <h2 className="text-[20px] font-semibold tracking-[-0.02em]">Select a mail</h2>
      <p className="text-[14px] text-muted-foreground mt-1.5 max-w-xs">
        Choose a thread on the left, or hit{' '}
        <kbd className="glass pill px-2 py-0.5 text-[11px] font-semibold mx-0.5">c</kbd> to compose.
      </p>
    </div>
  );
}
...
function LayoutShell() {
  ...
  const showingDocs = loc.startsWith('/docs');
  ...
  return (
    <div className="h-screen w-full flex relative">
      ...
      <main className="flex-1 flex min-w-0 relative lg:pl-[88px]">
        ...
        <div
          className={`
            ${showingMail || showingCompose || showingBriefing || showingSettings || showingArchive || showingCalendar || showingDocs ? 'hidden md:flex' : 'flex'}
            ${showingDocs || showingCalendar ? 'md:hidden' : 'md:flex'} flex-col w-full md:w-[360px] lg:w-[520px] xl:w-[560px] md:shrink-0
            relative lg:pr-2
          `}
        >
          <InboxList />
        ...
        <div
          className={`
            ${showingMail || showingCompose || showingBriefing || showingSettings || showingArchive || showingCalendar || showingDocs ? 'flex' : 'hidden md:flex'}
        ...
        >
          <Switch>
            ...
            <Route path="/calendar/new" component={CalendarPage} />
            <Route path="/calendar" component={CalendarPage} />
            <Route path="/docs/new" component={DocsDetail} />
            <Route path="/docs/:id" component={DocsDetail} />
